...
const scrapeGoBankingRates = async (query: string): Promise<ETFData[]> => {
  ...
  try {
    const url =
      "https://www.gobankingrates.com/investing/funds/best-high-dividend-etf/";
    ...
    $("table tbody tr").each((i, element) => {
      try {
        ...
        if (title && ticker && !isNaN(dividendYield) && dividendYield > 0) {
          ...
          const sector = sectorElement.length
            ? sectorElement.text().trim()
            : "Unknown";
          const regionElement = $(element).find("td:nth-child(5)");
          const region = regionElement.length
            ? regionElement.text().trim()
            : undefined;
