...
const scrapeSeekingAlpha = async (query: string): Promise<ETFData[]> => {
  try {
    ...
    const response = await retryRequest(() => axios.get(url, { headers: commonHeaders, timeout: 10000 }));
    ...
    $('div[data-test-id="etf-info"]').each((i, element) => {
      try {
        ...
        const ticker = $(element).find('span[data-test-id="symbol"]').text().trim();
        const yieldText = $(element).find('div:contains("Dividend Yield")').next().text().trim();
        ...
        console.error(`Error parsing SeekingAlpha ETF data:`, err);
      }
    });

    return results;
  } catch (error) {
    console.error(`Error scraping SeekingAlpha:`, error);
    return [];
  }
};
